...
window.awcInviteUser=async function(){const email=prompt('E-mailadres van de nieuwe gebruiker:');if(!email)return;const beheer=confirm('OK = beheerder\nAnnuleren = medewerker');const {data,error}=await sb.functions.invoke('awc-admin-users',{body:{action:'invite',email,role:beheer?'beheerder':'medewerker'}});if(error||data?.error)alert(data?.error||error.message);else{alert('Uitnodiging verstuurd.');loadMembers()}};
